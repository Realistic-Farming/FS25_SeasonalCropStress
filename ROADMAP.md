...
=======
- [~] SCS-023 finite irrigation water (2026-09-01): a pump carries a finite store in irrigation-hours; scheduled systems and Irrigate Now consume it, rain refills it, and a dry source stops its systems and says why. Engine half built on feat/SCS-023-finite-water: finite/unlimited source state with setSourceWaterRemaining, split binding (requireWater=false, same-farm/range/id ordering), the bounded planFiniteWater planner (grouped by source, pressure-scaled draw, rain refill, served-hours per system-hour never multiplied by area), collectScheduledHours for inactive mode, the shared applyGainToSystemCoverage positional write, getIrrigationSystemsRows + getIrrigationWaterSources farm-filtered read rows with stopReason, the fixed-1.0 Irrigate Now transaction (success/partial/no_ground/dry_source/no_source/wrong_farm, committedHours, applyWaterAtCell accepted-true contract), pump lifecycle (onLoad config, load/save remainder, finalize placement registration, owner rebind, dirty-flag stream), the finiteWater setting across defaults/panel/sync/SettingsHub, WeatherGuard-first rain read, the release-gate row finite_irrigation_water LOCKED, and the csWaterStatus diagnostic. 34 assertions in SCS-023-finite-irrigation-water_spec_test.lua. Release token waits for all five unlock observations. The FarmTablet readout is Wizard's half and travels with this.
>>>>>>> de7a998 (feat: SCS-023 finite irrigation water engine (planner, source state, Irrigate Now txn))
...
## 2026-09-09 (Fred): SCS-041 §8 schema-3 absorption persistence (slices 9a/9b, feat/SCS-041-thirsty-ground)
- [x] Slice 9a (83cd1e6): the pure pack/load port of the capacity ledger. `packAbsorptionWindow` packs the one carried current-hour window as a canonical, Adler-32-checked leaf (rows numerically sorted, `-` when grainless, adler over exact bytes); `loadAbsorptionWindow` restores replace-not-add with full validation (adler, row count, provider token/grain, window age: future rejects to stand-down, past expires, schema-2 migrates empty, UNCAPPED ignores), restores a live stand-down marker and its awaiting-hour/reason variants, and normalizes a future or unreadable-hour marker to the conservative bound; `pruneAbsorptionMissingFields` drops capacity rows for a sold/removed field. `scs041_persistence_test.lua` mirrors the spec bar Group I (87 assertions). Suite 1918/0.
- [x] Slice 9b (be44970): the absorption leaf nests in the real SCS-039 provider envelope core. `captureMoistureEnvelope` carries the optional leaf only for a CAPPED ledger that holds state (window, rows or a marker); `compactDigest` binds the leaf so identical mirrors dedupe and a drifted leaf changes the digest; `commitMoistureEnvelope` keeps the leaf on the PENDING_ONLY recovery row; `selectedAbsorptionLeaf` mirrors the spec bar Group M replace-not-add rule (M14-M20) on the real base-identity vocabulary. `scs041_envelope_leaf_test.lua` (26 assertions). Suite 1944/0 across 52 files.
- [ ] Remainder for the Iris pass: the SDS §8 on-disk carrier that reads a leaf back into `loadAbsorptionWindow` on a real reload (the engine-free envelope core has no live file-layer caller yet; `selectMoistureCarrier` is test-driven today). Check against the SDS when the E: design tree is reachable.
- [x] NEXT ACTION #3 decidable core (780738c, owner-ratified field-boundary correction): the geometry cache now retains the COMPLETE parcel polygon collection (every engine field sharing the farmland id), not a first match. `_getFieldPolygons` (cached collection, legacy single/refusal shapes preserved), `_pointInParcel` (point-in-ANY, gaps never filled), `materialiseRelief` and `fieldGeometryFingerprint` and `_uniqueFieldOwnerAt`/re-key over the collection, and `_acceptRunoffDestinationSpan` resolves the actual source polygon and requires the destination in that same polygon (a shared parcel is not permission to cross between its fields). `scs041_parcel_union_test.lua` drives two engine fields sharing one farmland (23 assertions); suite 1967/0 across 53 files.
- [ ] NEXT ACTION #3 SDS remainder (routed back): the native-map region-op union. `paintPolygon`/`applyDeltaToPolygon`/`readAverageOfPolygon` and their callers (migration base coat, native hourly weather delta, whole-field replacement, daily aggregate/settle re-derive) still take ONE polygon (`_getFieldVerts` first-polygon seam). Union rasterization, area-weighted union mean and once-per-cell handling on touching/overlapping polygons are SDS-governed and need the E: design text + in-game observation before implementation.
