...
- [x] Moisture map DMV render (2026-08-18): the map's Crop Moisture page stops drawing a grid of sampled tiles. It now renders through the engine density-map visualization overlay, the same pipeline Soil's PDA uses: a dedicated 4-bit display bitvector is band-painted per field polygon and stretched over the map rect as a crisp per-pixel overlay. The band map is separate from the persisted sim map on purpose, so bone-dry fields (raw state 0 in the sim encoding) stay visible and no sim data encoding changes. The sampled tile grid is kept as the engine-absent fallback.
