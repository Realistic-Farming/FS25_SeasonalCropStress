<#
.SYNOPSIS
    Extracts SeasonalCropStress-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the SeasonalCropStress mod, categorizing them by severity and type.
...
Write-Header "SeasonalCropStress Log Analyzer"
...
# Patterns to match
$patterns = @{
    # SeasonalCropStress specific patterns
    SeasonalCropStressLine = 'CropStress|FS25_SeasonalCropStress'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
    ...
    # Specific SeasonalCropStress patterns
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is SeasonalCropStress related
    if ($line -match $patterns.SeasonalCropStressLine) {
        $stats.SeasonalCropStressLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'SeasonalCropStress') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report = @()
$report += "=" * 60
$report += "SeasonalCropStress Error Report"
...
$report += "SeasonalCropStress-Related Lines: $($stats.SeasonalCropStressLines)"
