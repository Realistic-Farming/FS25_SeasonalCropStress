<#
.SYNOPSIS
    FS25_SeasonalCropStress Mod Validator (PowerShell version)
...
Write-Header "FS25_SeasonalCropStress Mod Validator"
...
foreach ($file in $LuaFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        if (-not $isComment -and $line -match '\bprint\s*\(' -and $line -notmatch 'SeasonalCropStress\.log' -and $RelPath -notmatch 'SeasonalCropStressCore\.lua') {
            Add-Warning -File $RelPath -Category "DEBUG_CODE" -Line $lineNum -Message "Raw print() - use CropStress.log* instead"
        }
    }
}
...
# Only flag cs_* keys as missing (base game keys are expected to be in game's i18n)
foreach ($file in $LuaFiles) {
    $RelPath = $file.FullName.Substring($ModPath.Length + 1)
    $content = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
    if (-not $content) { continue }
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        foreach ($match in $keyMatches) {
            ...
            # Only flag cs_* keys as errors (base game keys are assumed to exist)
            # Skip keys ending with _ (dynamic key construction like "cs_fluid_" .. fluidType)
            if ($key.StartsWith('cs_') -and -not $key.EndsWith('_') -and $key -notin $TransKeys) {
